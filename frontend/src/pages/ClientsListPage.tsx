...
export function ClientsListPage() {
  ...
  }, [accessToken, viewMode, debouncedQuery, searchBy, selectedCity, logout, navigate]);
    ...
    <div className="min-h-screen bg-[#f4f6fa] font-sans text-[#1c1c1c]">
      ...
      <nav className="bg-[#2c2759] text-white flex items-center justify-between px-6 py-3 shadow-md">
         ...
         <div className="flex items-center space-x-4">
            <div className="text-xs text-gray-300">Data last updated on : {data?.last_updated || 'Loading...'}</div>
            <div className="flex items-center bg-gray-800 rounded px-3 py-1 cursor-pointer" onClick={() => { logout(); navigate('/login'); }}>
              ...
              <div>
                ...
                <div className="text-[10px] text-gray-400">{data?.user.role === 'L' ? 'Leader' : data?.user.role === 'M' ? 'Manager' : 'Relationship Manager'}</div>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-4">
             <button onClick={() => navigate('/new-dashboard')} className="text-gray-500 hover:text-gray-800">
        ...
        <div className="flex justify-center mb-6">
           <div className="w-full max-w-sm bg-white rounded-xl py-6 border border-indigo-500 shadow-sm text-indigo-700 flex flex-col justify-center items-center text-center cursor-pointer transition-all">
              <div className="text-sm mb-2 font-medium text-indigo-500">All Clients</div>
              <div className="text-3xl font-bold text-gray-800">{data?.count || 0}</div>
           </div>
        </div>

        <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center flex-wrap gap-4 mb-6">
           <div className="text-sm text-gray-500 font-medium">Search by</div>
           <div className="flex border border-gray-200 rounded overflow-hidden text-sm">
             <button 
                onClick={() => setSearchBy('code')}
                className={`px-3 py-1.5 border-r border-gray-200 font-medium transition-all ${searchBy === 'code' ? 'text-orange-500 border-b-2 border-b-orange-500 bg-orange-50/30' : 'text-gray-500 hover:bg-gray-50'}`}
             >Client Code</button>
             <button 
                onClick={() => setSearchBy('pan')}
                className={`px-3 py-1.5 border-r border-gray-200 font-medium transition-all ${searchBy === 'pan' ? 'text-orange-500 border-b-2 border-b-orange-500 bg-orange-50/30' : 'text-gray-500 hover:bg-gray-50'}`}
             >Client PAN</button>
             <button 
                onClick={() => setSearchBy('name')}
                className={`px-3 py-1.5 border-r border-gray-200 font-medium transition-all ${searchBy === 'name' ? 'text-orange-500 border-b-2 border-b-orange-500 bg-orange-50/30' : 'text-gray-500 hover:bg-gray-50'}`}
             >Name</button>
             <button 
                onClick={() => setSearchBy('group')}
                className={`px-3 py-1.5 font-medium transition-all ${searchBy === 'group' ? 'text-orange-500 border-b-2 border-b-orange-500 bg-orange-50/30' : 'text-gray-500 hover:bg-gray-50'}`}
             >Group Code</button>
           </div>
           ...
           <div className="flex items-center space-x-3">
              ...
              <select 
                ...
                className="border-b border-gray-300 py-1.5 text-sm outline-none bg-transparent min-w-[120px] cursor-pointer"
              >
              ...
              <button 
                ...
                className="text-xs font-semibold border rounded-[3px] border-gray-400 px-2 py-[7px] text-indigo-600 hover:text-orange-600 transition-colors uppercase tracking-tight"
              >
                Clear
              </button>
           </div>

           {/* <button className="text-orange-600 text-3xl hover:opacity-80 ml-auto">
        ...
        <div className="bg-[#fcfdfd] rounded-lg border border-gray-200 shadow-sm overflow-hidden border-l-4 border-l-indigo-700 relative">
           ...
           <div className="overflow-x-auto">
             <table className="w-full text-sm text-left">
               <thead className="text-xs text-gray-500 bg-gray-100/80 border-b border-gray-200">
                 <tr>
                   <th className="px-6 py-4 font-medium whitespace-nowrap">Client Code</th>
                   <th className="px-6 py-4 font-medium whitespace-nowrap">Client PAN</th>
                   <th className="px-6 py-4 font-medium whitespace-nowrap">Client Name</th>
                   <th className="px-6 py-4 font-medium whitespace-nowrap">Group Code</th>
                   <th className="px-6 py-4 font-medium whitespace-nowrap text-gray-800 flex items-center cursor-pointer">
                     Total AUM (₹) <i className="fas fa-sort text-orange-400 ml-1"></i>
                   </th>
               ...
               <tbody className="divide-y divide-gray-100">
                 {data?.records && data.records.length > 0 ? (
                   data.records.map((row, i) => (
                     <tr key={i} className="hover:bg-gray-50 transition-colors h-20">
                       <td className="px-6 py-4 text-gray-700">{row.code}</td>
                       <td className="px-6 py-4 text-gray-700">{row.pan}</td>
                       <td className="px-6 py-4 max-w-[200px]">
                         <div className="text-blue-500 font-medium text-xs hover:underline cursor-pointer uppercase leading-tight line-clamp-2">{row.name}</div>
                       </td>
                       <td className="px-6 py-4 font-medium text-gray-700">{row.group}</td>
                       <td className="px-6 py-4 font-medium text-gray-800">
                         ₹ {row.aum.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                       </td>
                       <td className="px-6 py-4 text-gray-600">{row.city}</td>
                       {/* <td className="px-6 py-4 text-right whitespace-nowrap space-x-2">
                         <button className="text-green-500 hover:text-green-600 border border-green-500 rounded p-1.5 transition-colors"><i className="fas fa-phone-alt"></i></button>
                         <button className="text-orange-400 hover:text-orange-500 border border-orange-400 rounded p-1.5 transition-colors"><i className="far fa-envelope"></i></button>
                         <button className="text-orange-400 hover:text-orange-500 border border-orange-400 rounded p-1.5 transition-colors"><i className="far fa-user"></i></button>
                       </td> */}
                     </tr>
                   ))
                 ) : (
                   <tr>
                     <td colSpan={7} className="px-6 py-12 text-center text-gray-400 italic">
                       {loading ? 'Fetching clients...' : 'No clients found matching your criteria.'}
                     </td>
                   </tr>
                 )}
               </tbody>
