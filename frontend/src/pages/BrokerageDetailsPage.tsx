...
export function BrokerageDetailsPage() {
  ...
  const [activeCard, setActiveCard] = useState<"equity" | "mf" | "others">(
    "equity",
  );
  ...
  useEffect(() => {
    ...
    if (
      viewMode === "team" &&
      cd.rm_performance?.labels?.length &&
      rmCanvasRef.current
    ) {
    ...
    if (
      viewMode === "team" &&
      cd.ma_performance?.labels?.length &&
      maCanvasRef.current
    ) {
    ...
    if (
      viewMode === "team" &&
      cd.segment_analysis?.labels?.length &&
      segmentCanvasRef.current
    ) {
      chartsRef.current.seg = new Chart(segmentCanvasRef.current, {
        ...
        data: {
          ...
          datasets: [
            {
              data: cd.segment_analysis.cash || [50, 50],
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-semibold uppercase tracking-tight font-nunito">
              Brokerage {tab === "summarised" ? "(YTD)" : "(Detailed)"}
            </h1>
          </div>
          <div className="bg-white rounded-full p-1 border border-gray-200 shadow-sm flex items-center">
            <button
              onClick={() => setViewMode("team")}
              className={`px-4 py-1.5 font-nunito rounded-full text-sm font-medium transition-all ${viewMode === "team" ? "bg-gray-500 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100"}`}
            >
              <i className="fas fa-users mr-2"></i> Team
            </button>
            <button
              onClick={() => setViewMode("self")}
              className={`px-4 py-1.5 font-nunito rounded-full text-sm font-medium transition-all ${viewMode === "self" ? "bg-gray-500 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100"}`}
            >
        ...
        {/* Cards */}
        <div className="flex flex-wrap gap-4 mb-4 font-nunito">
          {[
            {
              id: "equity",
              title:
                tab === "summarised" ? "Equity (YTD)" : "Equity (Selected)",
              val: formatCurrency(data?.totals.equity || 0),
            ...
            },
            {
              ...
              val: formatCurrency(data?.totals.mf || 0),
              active: activeCard === "mf",
            },
            {
              id: "others",
              title:
                tab === "summarised" ? "PMS/AIF (YTD)" : "PMS/AIF (Selected)",
              val: formatCurrency(data?.totals.others || 0),
              active: activeCard === "others",
            },
          ].map((card) => (
            <div
              key={card.id}
            ...
            >
              <div className="flex flex-col justify-center items-center text-center">
                <div
                  className={`text-sm mb-2 font-nunito font-semibold uppercase tracking-wider ${card.active ? "text-indigo-600" : "text-gray-400"}`}
                >
                  {card.title}
                </div>
                <div
                  className={`text-3xl font-nunito font-bold ${card.active ? "text-gray-900" : "text-gray-700"}`}
                >
                  ₹ {card.val}
                </div>
              </div>
        ...
        <div className="flex justify-end mb-1">
          <div className="flex border border-gray-200 rounded-full overflow-hidden bg-white shadow-sm">
            <button
              onClick={() => setTab("summarised")}
              className={`px-6 py-1.5 font-nunito text-sm font-bold transition-all ${tab === "summarised" ? "bg-orange-50/30 text-orange-600" : "text-gray-500 hover:bg-gray-50"}`}
            >
              Summarised
            </button>
            <button
              onClick={() => setTab("detailed")}
              className={`px-6 py-1.5 font-nunito text-sm font-bold transition-all ${tab === "detailed" ? "bg-orange-50/30 text-orange-600" : "text-gray-500 hover:bg-gray-50"}`}
            >
              Detailed
            </button>
          </div>
        </div>
        <div className="text-right text-[10px] text-gray-400 mb-6 uppercase font-bold tracking-widest font-nunito">
          {tab === "summarised"
            ? "Showing full YTD data"
            : "Showing data for the selected filters and date range"}
        </div>

        {/* Detailed Filters Section */}
        {tab === "detailed" && (
          <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm space-y-6 mb-6 animate-in fade-in slide-in-from-top-2 font-nunito">
            {viewMode === "team" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col">
                  <label className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">
                    Manager
                  </label>
                  <select
                    ...
                    className="border border-gray-200 rounded px-3 py-2 text-sm outline-none focus:border-indigo-500 bg-transparent"
                  >
            ...
            <div
              className={`grid grid-cols-1 gap-6 items-end ${viewMode === "team" ? "md:grid-cols-4" : "md:grid-cols-3"}`}
            >
              <div className="flex flex-col">
                <label className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">
                  From Date
                ...
                <input
                  ...
                  className="border border-gray-200 rounded px-3 py-1.5 text-sm outline-none focus:border-orange-500"
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">
              ...
              {viewMode === "team" && (
                <div className="flex flex-col">
                  ...
                  <select
                    ...
                    className="border border-gray-200 rounded px-3 py-1.5 text-sm outline-none focus:border-indigo-500 bg-transparent"
                  >
              ...
              <button
                ...
                className="h-[38px] text-xs font-bold text-indigo-600 border border-indigo-100 rounded-lg hover:bg-indigo-50 uppercase tracking-tighter transition-all"
              >
        ...
        {tab === "detailed" && data && (
          <div className="space-y-8 mb-10">
            <div
              className={`grid grid-cols-1 gap-6 ${viewMode === "team" ? "lg:grid-cols-2" : "lg:grid-cols-1"}`}
            >
              {viewMode === "team" && (
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-[350px]">
                  <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">
                    RM Performance - Brokerage
                  </div>
                  <div className="h-[260px]">
                    <canvas ref={rmCanvasRef} />
                  </div>
                </div>
              )}
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-[350px]">
                ...
                <div className="h-[260px]">
                  <canvas ref={topClientsCanvasRef} />
                </div>
              </div>
            </div>
            ...
            {viewMode === "team" && (selectedRM || selectedManager) && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-[350px]">
                  <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">
                    MA Performance under {selectedRM || selectedManager}
                  </div>
                  <div className="h-[260px]">
        ...
        <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center flex-wrap gap-4 mb-6">
          <div className="text-xs text-gray-400 font-bold uppercase tracking-widest font-nunito">
            Search By
          </div>
          <div className="flex border border-gray-200 rounded overflow-hidden text-sm font-bold">
            ...
            <button
              ...
              className={`px-3 py-1.5 font-nunito transition-all ${searchBy === "name" ? "text-orange-500 border-b-2 border-b-orange-500 bg-orange-50/30" : "text-gray-500 hover:bg-gray-50"}`}
            >
              Name
            </button>
          </div>
          ...
          <div className="flex-1 min-w-[150px] border-b border-gray-300 flex items-center px-4">
            <input
              ...
              className="w-full py-1.5 font-nunito outline-none text-sm text-center text-gray-700 bg-transparent font-medium"
            />
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-[#fcfdfd] rounded-lg border border-gray-200 shadow-sm overflow-hidden relative">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left whitespace-nowrap">
              <thead className="text-[13px] text-gray-400 bg-gray-50/80 border-b border-gray-200 uppercase tracking-widest font-bold font-nunito">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right">
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-sm font-bold text-gray-800 tracking-wide mt-1">
                        {formatCurrency(
                          activeCard === "equity"
                            ? data?.totals.equity || 0
                            : activeCard === "mf"
                              ? data?.totals.mf || 0
                              : data?.totals.others || 0,
                        )}
                      </span>
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-gray-100 font-nunito">
                {data?.records && data.records.length > 0 ? (
                  data.records.map((row, i) => (
                    <tr
                      key={i}
                      className={`hover:bg-gray-50 transition-colors border-l-4 border-l-indigo-700 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}
                    >
