...
type LandingSummaryResponse = {
  ...
  totals: {
    ...
    pms_aif: { aum: number; brokerage: number; clients: number };
    active_clients: number;
    dormant_clients: number;
    equity_aum_detail: number;
    mf_aum_detail: number;
    pms_aum_detail: number;
    aif_aum_detail: number;
  };
  last_updated: string;
};
...
export function NewLandingPage() {
  ...
  // Scoped key for localStorage to keep it user-specific
  const storageKey = authUser ? `default_tab_${authUser.username}` : null;

  // Use a ref to track if we've already set the initial tab from preferences
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      // Set initial tab priority: 1. LocalStorage, 2. Backend, 3. Default 'Overall'
      if (!initialLoadDone.current) {
        const localPref = storageKey ? localStorage.getItem(storageKey) : null;
        if (localPref) {
          setActiveTab(localPref);
          // Important: Overwrite backend's default_tab in our local state to keep indicators in sync
          summaryData.user.default_tab = localPref;
        } else if (summaryData.user.default_tab) {
          setActiveTab(summaryData.user.default_tab);
        }
  ...
  const fetchHierarchy = async () => {
    ...
    try {
      const res = await axios.get<HierarchyData>(
        "/api/dashboard/hierarchy-list/",
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        },
      );
  ...
  const setDefaultTab = async () => {
    ...
    // 1. Update UI and LocalStorage Immediately (Instant Success)
    if (storageKey) {
    ...
    // Update local data state to reflect the new default
    setData((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
        user: { ...prev.user, default_tab: activeTab },
      };
    });

    // 2. Sync to backend in background (Bypass lock frustration)
    try {
      await axios.post(
        "/api/user/preference/",
      ...
      // Log silently - we don't want to show "Retry Fail" if local save worked
      console.warn("Background sync of default tab failed:", error);
    }
  };
  ...
  const getActiveTotals = () => {
    ...
    if (activeTab === "Other Products") return data.totals.pms_aif;
  ...
  return (
    <div className="min-h-screen bg-[#f4f6fa] font-sans text-[#1c1c1c]">
      ...
      <nav className="bg-[#2c2759] text-white flex items-center justify-between px-30 py-3 shadow-md">
        <div className="flex items-center space-x-6">
          ...
          <div className="hidden md:flex space-x-1">
            <button className="px-4 py-2 font-nunito bg-indigo-700/35  text-sm font-medium border-b-2 border-orange-500">
              Dashboard
            </button>
            <button className="px-4 py-2 font-nunito hover:bg-indigo-700/30  text-sm text-gray-300">
              MIS
            </button>
            {/* <button className="px-4 py-2 hover:bg-indigo-700/30  text-sm text-gray-300">
              Markets
            </button>
            <button className="px-4 py-2 hover:bg-indigo-700/30  text-sm text-gray-300">
              Invest
        ...
        <div className="flex items-center space-x-4">
          <div
            className="w-8 h-8 bg-gray-500 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-400 transition-colors"
            onClick={() => {
              logout();
              navigate("/login");
            }}
      ...
      <main className="max-w-7xl mx-auto px-14 py-6">
        {/* Header Section */}
        <div className="flex justify-between items-center mb-6 font-nunito">
          <div className="flex items-center space-x-3">
            ...
            >
              <img
                src="https://aaa.iiflcapital.com/assets/svg/user-search.svg"
                alt="emp search"
                className="w-8 h-8"
              />
            </span>
        ...
        <div className="mb-6 font-nunito">
          ...
          >
            <i
              className={`fas ${showDateFilters ? "fa-chevron-up" : "fa-chevron-down"} mr-2`}
            ></i>
            Date Filters
          </button>
            ...
            <div className="mt-2 bg-white rounded-xl px-6 py-4 border border-gray-200 shadow-sm flex items-center gap-6 animate-in fade-in slide-in-from-top-2">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className="border border-gray-200 rounded px-3 py-1.5 text-sm outline-none focus:border-indigo-500"
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-wider">
              ...
              <button
                ...
                className="mt-5 text-xs font-bold text-gray-500 hover:text-orange-600 uppercase"
              >
                Clear
              </button>
              <div className="ml-auto text-[10px] text-gray-400 self-end mb-1">
        ...
        {/* Tabs */}
        <div className="flex space-x-8 border-b border-gray-200 mb-6 relative font-nunito">
          {["Overall", "Mutual Funds", "Equity", "Other Products"].map(
            (tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`py-3 text-sm font-bold border-b-2 uppercase font-nunito tracking-normal transition-all ${activeTab === tab ? "border-indigo-600 text-indigo-700" : "border-transparent text-gray-400 hover:text-gray-700"}`}
              >
                {tab}{" "}
                {data?.user.default_tab === tab && (
                  <i className="fas fa-thumbtack ml-1 text-xs"></i>
                )}
              </button>
            ),
          )}
          <div className="ml-auto py-3">
            <button
              ...
              disabled={
                activeTab === data?.user.default_tab || saveStatus || saveError
              }
              className={`text-xs font-bold uppercase tracking-widest flex items-center transition-all ${activeTab === data?.user.default_tab ? "text-gray-300 cursor-default" : saveError ? "text-red-500" : saveStatus ? "text-green-600" : "text-indigo-600 hover:text-indigo-800"}`}
            >
              <i
                className={`fas ${saveError ? "fa-exclamation-circle" : saveStatus ? "fa-check" : "fa-thumbtack"} mr-1`}
              ></i>
              {activeTab === data?.user.default_tab
                ? "Default"
                : saveError
                  ? "Retry Fail"
                  : saveStatus
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold tracking-tight text-gray-800 font-nunito">
                Business Overview
              </h2>
            ...
            {/* Main 3 Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-nunito">
              <div
                onClick={() =>
                  navigate("/aum-details", {
                    state: {
                      initialCard:
                ...
                className="bg-violet-50/50 rounded-xl p-6 border border-indigo-200 shadow-sm cursor-pointer hover:shadow-md transition-all relative overflow-hidden group"
              >
                ...
                <div className="w-8 h-8 rounded bg-purple-100 text-purple-600 flex items-center justify-center mb-6">
                  <i className="fas fa-cube"></i>
                </div>
                <div className="text-purple-700 font-bold text-lg flex items-center uppercase tracking-tighter">
                  AUM{" "}
                  <i className="fas fa-chevron-right ml-2 text-xs opacity-50 group-hover:translate-x-1 transition-all"></i>
                </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="text-orange-700 font-bold text-lg flex items-center uppercase tracking-tighter">
                    {activeTab === "Other Products" ? "Payout" : "Brokerage"}{" "}
                    <i className="fas fa-chevron-right ml-2 text-xs opacity-50 group-hover:translate-x-1 transition-all"></i>
                  </div>
            ...
            {/* Bottom 4 Grid Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 font-nunito">
              {[
                {
                  title: "Equity AUM",
                  val: formatCurrency(data?.totals.equity_aum_detail || 0),
                },
                {
                  title: "MF AUM",
                  val: formatCurrency(data?.totals.mf_aum_detail || 0),
                },
                {
                  title: "PMS AUM",
                  val: formatCurrency(data?.totals.pms_aum_detail || 0),
                },
                {
                  title: "AIF AUM",
                  val: formatCurrency(data?.totals.aif_aum_detail || 0),
                },
                {
                  title: "Active Clients",
                  val: data?.totals.active_clients || 0,
                  info: "Transacted in last 12 months",
                },
                {
                  title: "Dormant Clients",
                  val: data?.totals.dormant_clients || 0,
                  info: "No transaction in last 24 months",
                },
              ].map((card, i) => (
                <div
                  key={i}
                  className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex flex-col justify-center hover:border-indigo-200 transition-colors"
                >
                  <div className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1 flex items-center">
                    {card.title}{" "}
                    {card.info && (
                      <i
                        className="fas fa-info-circle ml-1 text-blue-400 cursor-help"
                        title={card.info}
                      ></i>
                    )}
                  </div>
          ...
          <div className="space-y-6 font-nunito">
            ...
            {/* My Business Planner */}
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden relative group cursor-pointer hover:border-blue-200 transition-all h-full">
              <div className="absolute top-0 left-0 bg-blue-500 text-white text-[9px] font-bold px-3 py-1 rounded-br-lg z-10 uppercase tracking-widest">
                EXCLUSIVE
              </div>
              <div className="p-5 pt-8 flex items-center">
                ...
                <div>
                  <h3 className="font-bold text-gray-800 text-[15px] uppercase tracking-tight">
                    Business Planner
                  </h3>
                  <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-widest flex items-center">
                    Track RM Performance{" "}
                    <i className="fas fa-chevron-right ml-1 opacity-50 group-hover:translate-x-1 transition-all"></i>
                  </p>
                </div>
              </div>
              <div className="p-5 pt-8 flex items-center justify-center text-gray-300">
                COMING SOON...
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in transition-all">
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-indigo-50/30">
              <h3 className="text-lg font-bold text-indigo-900 uppercase tracking-tight">
                View Hierarchy
              </h3>
              <button
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              {modalLoading ? (
                ...
                <div className="space-y-8">
                  {/* Managers Section */}
                  <div>
                    <h4 className="text-[10px] font-bold text-orange-500 uppercase tracking-[0.2em] mb-4 border-b border-orange-100 pb-1">
                      Managers
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {hierarchyData?.managers.length ? (
                        hierarchyData.managers.map((name) => (
                          <span
                            key={name}
                            className="px-3 py-1.5 bg-gray-50 text-gray-700 rounded-lg text-xs font-semibold border border-gray-100 shadow-sm"
                          >
                            {name}
                          </span>
                        ))
                  ...
                  {/* RMs Section */}
                  <div>
                    <h4 className="text-[10px] font-bold text-indigo-500 uppercase tracking-[0.2em] mb-4 border-b border-indigo-100 pb-1">
                      Relationship Managers
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {hierarchyData?.rms.length ? (
                        hierarchyData.rms.map((name) => (
                          <span
                            key={name}
                            className="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-semibold border border-indigo-100 shadow-sm"
                          >
                            {name}
                          </span>
                        ))
                  ...
                  {/* MAs Section */}
                  <div>
                    <h4 className="text-[10px] font-bold text-green-600 uppercase tracking-[0.2em] mb-4 border-b border-green-100 pb-1">
                      MF Advisors
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {hierarchyData?.mas.length ? (
                        hierarchyData.mas.map((name) => (
                          <span
                            key={name}
                            className="px-3 py-1.5 bg-green-50 text-green-700 rounded-lg text-xs font-semibold border border-green-100 shadow-sm"
                          >
                            {name}
                          </span>
                        ))
